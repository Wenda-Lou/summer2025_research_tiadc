...
def estimate_channel(
    ...
    for pass_index in range(max(1, passes)):
        ...
        # Pooling hook: when a joint accumulator is attached, record this frame's
        # event windows instead of finishing the amplitude fit locally.  See
        # JointEventAccumulator -- stacking windows from many frames is what gives
        # the joint estimator enough events.
...
class CalibrationState:
    ...
    skew_limit_ps: float = 165.0
    """Clamp on :attr:`skew_cmd_ps`, in picoseconds.

    The AD9695 delay is one-sided, so ``capture.AdcClockDelay`` parks channel A
    at its bias (165 ps by default) and moves B relative to it.  B can therefore
    only realise roughly +-165 ps of *differential* delay, and the loop must not
    integrate past that: on a stalled capture the measured skew error never
    responds to the command, and an unclamped integrator walks the actuator into
    its rail (a 296-iteration run reached a commanded 360.8 ps against a 165 ps
    authority, leaving channel B pinned at full delay).  Clamping keeps the state
    honest; :attr:`skew_saturated` records that it happened, so the run says so
    instead of quietly losing authority."""

    skew_saturated: int = 0
    """Number of updates that hit :attr:`skew_limit_ps`."""
    ...
    def update(self, est: BlockEstimate) -> dict:
        ...
        if np.isfinite(est.skew_mismatch_ps):
            ...
            # The command is clamped to what the actuator can actually realise;
            # without that, an error the hardware cannot respond to (a stalled
            # capture, or a genuinely unreachable target) integrates forever and
            # parks channel B at its rail.
            wanted = self.skew_cmd_ps - self.mu_skew * e_skew
            clamped = float(np.clip(wanted, -self.skew_limit_ps, self.skew_limit_ps))
            if clamped != wanted:
                self.skew_saturated += 1
            self.skew_cmd_ps = clamped
